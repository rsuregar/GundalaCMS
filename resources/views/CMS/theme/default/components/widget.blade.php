<div class="col-md-4 pl-4 pr-4" style="margin-top:-12px">
    <hr class="featurette-divider d-block d-sm-none">
    @forelse (\App\Widget::where('status', 1)->where('show_at', 'like', '%home%')->orderBy('ordered')->get() as $item)
            ...
            @case(4)
                ...
                <section id="content" class="mb-5">
                    ...
                    @foreach (\App\Category::whereHas('blog')->orderBy('id', $item->widget_content)->get() as $a)
                        <a href="{{ route('category.show', $a->slug) }}" type="button" class="btn btn-sm btn-primary" style="border-radius:10px">
