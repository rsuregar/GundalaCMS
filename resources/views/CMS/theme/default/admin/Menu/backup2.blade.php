                    ...
                    <nav>
                      ...
                      <div class="tab-content" id="nav-tabContent">
                        ...
                        <form>
                                ...
                                <div class="collapse {{ ($action == 'create') ? 'show':'' }}">
                                ...
                                <div class="collapse {{ ($action == 'edit') ? 'show':'' }}">
                                    <div class="card card-body">
                                        <form action="{{ route('menu.update', !empty($get) ? $get->id:'') }}" method="post">
                                            @csrf
                                            @method('PATCH')
                                            <table class="table table-sm table-bordered mt-3">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr class="text-center text-danger">
                                                        <th colspan="5">Oups... Data tidak ditemukan</th>
                                                    </tr>
                                                    @else
                                                        <tr>
                                                            <td>
                                                                <input type="hidden" name="action" value="edit">
                                                                <input type="text" name="title" class="form-control form-control-sm" value="{{ $get->title }}">
                                                            </td>
                                                            <td>
                                                                <select name="menu_location" class="form-control form-control-sm" required>
                                                                    <option value="">Pilih lokasi</option>
                                                                    @foreach ($lokasi as $a)
                                                                    <option {{ $get->menu_location == $a ? 'selected':'' }} value="{{ $a }}">{{ $a }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </td>
                                                            <td>
                                                                <select name="menu_position" class="form-control form-control-sm" required>
                                                                    <option value="">Pilih posisi</option>
                                                                    @foreach ($posisi as $b)
                                                                    <option {{ $get->menu_position == $b ? 'selected':'' }} value="{{ $b }}">{{ $b }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </td>
                                                            <td>
                                                                <select name="is_active" class="form-control form-control-sm" required>
                                                                    ...
                                                                    @foreach ($status as $c)
                                                                    <option {{ $get->is_active == $c ? 'selected':'' }} value="{{ $c }}">{{ $c == 1 ? 'Tampilkan':'Tidak tampil' }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </td>
                                                            <td class="text-center">
                                                                <form id="delete" action="{{ route(Request::segment(2).'.destroy', $get->id) }}" method="POST" style="display: none;">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button class="btn btn-link text-danger">Trash</button>
                                                                </form>
                                                            </td>
                                                        </tr>
                                                    @endempty
